...
LLM_MODEL = "gemini-2.5-pro" # version from june 2025 or first stable version
...
OUT_PATH = os.path.join(RESULTS_DIR, f"splittability_eval_gemini_2_5_pro.csv")
...
def load_submissions_from_csv():
    """
    Lädt Submissions, plant Gruppen basierend auf Clustergrößen und wählt dann
    zufällige Submissions aus dem jeweiligen Cluster für jede geplante Gruppe aus.
    """
    clusters = {}
    ...
    # ▸ Clusterverteilung planen (max. Submissions pro Prompt beachten)
    cluster_plan = []
    ...
    # ▸ Submissions für jede geplante Gruppe einsammeln
    grouped = []
    for label, limit in cluster_plan:
        # Wähle zufällige Submissions *aus dem aktuellen Cluster*
        available_subs = clusters[label]
        chosen_subs = list(random.sample(available_subs, min(limit, len(available_subs))))
        if chosen_subs:
            grouped.append((label, chosen_subs))
...
def main():
    ...
    grouped = load_submissions_from_csv()
    if not grouped:
        print("[WARN] Keine Submissions zum Bewerten gefunden.")
        return
    print(f"[INFO] {len(grouped)} Cluster-Gruppen werden bewertet …")
    ...
    def worker(cluster_label, subs):
        ...
        try:
            ...
            try:
                ...
                print(f"[WARN] Parsing für Cluster '{cluster_label}' fehlgeschlagen: {e}")
                reason = f"PARSING_ERROR: {e}\n\nRAW_RESPONSE:\n{raw_response}"
        except Exception as e:
            print(f"[ERROR] Kritischer Fehler bei Cluster '{cluster_label}': {e}")
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for i, fut in enumerate(as_completed(futures), start=1):
            ...
            print(f"[INFO] Bewertung {i}/{len(grouped)} abgeschlossen (Cluster „{result_dict['cluster_label']}“, Status: {status}).")
    ...
    print(f"[INFO] {len(results_to_save)} Gruppenbewertungen gespeichert (Datei: {OUT_PATH}).")

if __name__ == "__main__":
    print("─── STARTE SPLITTABILITY-EVALUATION ───")
    try:
        main()
    finally:
