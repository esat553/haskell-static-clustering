...
RANDOM_CSVS = {
    "Claude 4 Sonnet (Random)": "results/llm_as_judge_random_sonnet_4.csv",
    "Gemini 2.5 Flash (Random)": "results/llm_as_judge_random_gemini_2_5_flash.csv",
    "GPT-4o (Random)": "results/llm_as_judge_random_gpt_4o.csv",
}
RESULTS_DIR = "results"
...
def analyze_cross_model(llm_data, report_lines):
    # Nur wenn alle LLMs vorhanden sind
    keys = list(llm_data.keys())
    if len(keys) < 2:
        return
    # Gemeinsame submissionIDs
    merged = None
    for k in keys:
...
def analyze_random_baseline(llm_data, random_data, report_lines):
    ...
    for k in llm_data:
        ...
        # Finde passendes Random-Modell
        rand_key = [rk for rk in random_data if k in rk or rk.startswith(k.split()[0])] # fuzzy
        if not rand_key:
...
def main():
    ...
    # Optional: Zeige die wichtigsten Ergebnisse auch in der Konsole
