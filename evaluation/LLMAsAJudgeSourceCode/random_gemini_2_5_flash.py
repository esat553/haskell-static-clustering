...
OUT_PATH = os.path.join(RESULTS_DIR, f"llm_as_judge_gemini_2_5_flash_random.csv")
...
CATEGORIES = load_categories(CATEGORIES_LIST_FILE)
...
def main():
    print(f"[INFO] RANDOM-BASELINE mit Gemini-Modell: {LLM_MODEL}")
    ...
    with open(CSV_FILE_PATH, encoding="utf-8") as infile, open(OUT_PATH, "w", newline='', encoding="utf-8") as outfile:
        ...
        rows = [
            (row['submissionID'], row['source_code'], row['compiler_output'])
            for row in reader
        ][:MAX_SUBMISSIONS]
        total = len(rows)
        def worker(row):
            ...
            rnd_label = random_cluster()
            prompt, temperature = generate_prompt(src_clean, err, rnd_label)
            raw_response = ask_llm(model, temperature, prompt)
            rating, reason = parse_response(raw_response)
            return {
                'submissionID': sid,
                'cluster_label': rnd_label,
                'rating': rating,
                'reason': reason,
