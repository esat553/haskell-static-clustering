...
LLM_MODEL = "gemini-2.5-flash" # version from june 2025 or first stable version
...
OUT_PATH          = os.path.join(RESULTS_DIR, f"llm_as_judge_gemini_2_5_flash.csv")
...
def generate_prompt(src: str, err: str, label: str):
    ...
    cluster_label = prompt_data["cluster_label"]
...
def ask_llm(model, temperatureValue, prompt: str, backoff=2):
    attempt = 0
    while True:
        try:
            response = model.generate_content(
                prompt,
                generation_config=genai.types.GenerationConfig(temperature=temperatureValue)
            )
...
def main():
    ...
    if not api_key:
        raise EnvironmentError(f"Fehlende Umgebungsvariable {GOOGLE_API_KEY_ENV_VAR}")
