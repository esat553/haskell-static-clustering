# haskell_cluster_pipeline.py (ADAPTED VERSION - Error Message Embedding Focus)
...
CONFIG = {
    ...
    "FILES": {
        ...
        "prepared_data": "embedding_results/prepared_for_embedding_errors.csv",
        "embeddings_npy": "embedding_results/embeddings_errors.npy",
        "embeddings_df": "embedding_results/embeddings_errors.parquet",
        "umap_2d": "embedding_results/umap_2d_errors.npy",
        "umap_model": "embedding_results/umap_model_errors.pkl",
        "cluster_labels": "embedding_results/cluster_labels_errors.npy",
        "final_clustered_data": "embedding_results/clustered_data_errors.csv"
    }
}
...
def load_and_prepare_data(cfg):
    ...
    if 'id' not in df.columns or 'compiler_output' not in df.columns:
        raise ValueError("CSV must contain 'id' and 'compiler_output' columns.")
    ...
    df['combined_text'] = df['compiler_output'].apply(clean_error_message_for_embedding)
    df.rename(columns={'id': 'submissionID'})[['submissionID', 'combined_text']].to_csv(files_cfg['prepared_data'], index=False)
    print(f"✅ {len(df)} error messages prepared and saved to '{files_cfg['prepared_data']}'.")
    return df
...
def generate_embeddings(cfg):
    ...
    print(f"Generating embeddings for {len(texts)} error messages...")
...
def run_dashboard(cfg):
    ...
    st.title("Interactive Cluster Analysis of Haskell Error Messages")
    ...
    if not filtered_df.empty:
        st.markdown("### Sample Error Message from Plot")
        if options := filtered_df['submissionID'].tolist():
            ...
            if "source_code" in row:
                ...
                st.code(row["source_code"], language='haskell')
            else:
                st.info("No source code available for this submission.")
